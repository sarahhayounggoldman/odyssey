...
function likePost(postId) {
    ...
    $.post("/likeAjax/" + postId, { postId: postId })
        .then(processAction);
}
...
document.addEventListener('DOMContentLoaded', function () {
    const dateTimeElements = document
...
$(document).ready(function () {
    $('body').on('click', '.sendComment', function () {
        ...
        var commentText = $(this).closest("form")
            .find('input[name="comment"]').val();
        addComment(postId, commentText);
    });
});

function addComment(postId, comment) {
    $.post("/commentAjax/" + postId, { comment: comment })
        .then(processActionComments);
}
...
function processActionComments(resp) {
    if (resp.error) {
        ...
        const postContainer = $(`[data-post-id="${resp.postId}"]`)
            .closest(".post-container");

        const commentDiv = $('<div>').addClass('comment');
        const commentP = $('<p>').text(resp.comment.text);
        const commentUser = $('<small>')
            .text(`Comment by: ${resp.comment.userId}`);
